...
import { ArrowLeft, CreditCard, Mail, UserRound } from "lucide-react";
...
export default async function TransactionDetailPage({ params }: DetailPageProps) {
  ...
  const result = await supabase.from("transactions").select("id, customer_id, amount, currency, status, failure_reason, external_reference, created_at, customers(id, name, email, phone, created_at), recovery_actions(id, strategy, status, details, executed_at, created_at), ai_decisions(id, recovery_probability, reasoning, model, created_at)").eq("id", id).single();
  ...
  if (transaction.status === "recovered") auditEvents.push({ title: "Payment recovered", description: "The recovery flow completed successfully.", timestamp: formatDate(lastAction?.executed_at), type: "success" });

  return <DetailShell>
    <Link href="/transactions" className="inline-flex items-center gap-2 text-sm text-slate-400 transition-colors hover:text-white"><ArrowLeft size={16} /> Back to transactions</Link>
    <div className="mt-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
      <div><p className="font-mono text-xs text-[#b8f36b]">{transaction.external_reference}</p><h1 className="mt-3 text-4xl font-semibold tracking-tight">Transaction details</h1><p className="mt-3 text-slate-400">{formatDate(transaction.created_at)}</p></div>
      <Badge className={transaction.status === "recovered" ? "border-emerald-400/20 bg-emerald-400/10 text-emerald-300" : "border-red-400/20 bg-red-400/10 text-red-300"}>{transaction.status}</Badge>
    </div>
    <div className="mt-10 grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
      <div className="space-y-6">
        <Card><CardContent><SectionTitle icon={UserRound} title="Customer information" /><div className="mt-6 grid gap-5 sm:grid-cols-2"><Detail label="Customer ID" value={customer?.id?.slice(0, 12) ?? "Not recorded"} mono /><Detail label="Name" value={customer?.name ?? "Unknown customer"} /><Detail label="Email" value={customer?.email ?? "Not recorded"} /><Detail label="Customer type" value={customerTypeFor(successfulPayments, totalSpent)} /><Detail label="Previous successful payments" value={successfulPayments.toString()} /><Detail label="Previous failed payments" value={failedPayments.toString()} /><Detail label="Total spent" value={formatINR(totalSpent)} /></div></CardContent></Card>
        <Card><CardContent><SectionTitle icon={CreditCard} title="Payment information" /><div className="mt-6 grid gap-5 sm:grid-cols-2"><Detail label="Transaction ID" value={transaction.id.slice(0, 12)} mono /><div><p className="text-xs uppercase tracking-wider text-slate-500">Amount</p><p className="mt-2 text-sm text-slate-200">{formatINR(Number(transaction.amount))}</p>{transaction.status !== "recovered" && <div className="mt-3"><RetryPaymentButton transactionId={transaction.id} reference={transaction.external_reference} amount={Number(transaction.amount)} currency={transaction.currency} customerName={customer?.name ?? "Customer"} customerEmail={customer?.email ?? ""} customerPhone={customer?.phone} /></div>}</div><Detail label="Payment method" value={paymentMethodFor(transaction.failure_reason)} /><Detail label="Timestamp" value={formatDate(transaction.created_at)} /><Detail label="Failure reason" value={transaction.failure_reason ?? "Not recorded"} /><Detail label="Attempt count" value={actions.length.toString()} /></div></CardContent></Card>
        <Card><CardContent><h2 className="text-lg font-semibold">Recovery history</h2><Separator className="my-5" />{actions.length ? <div className="space-y-4">{actions.map((action) => <div key={action.id} className="flex items-start justify-between gap-4"><div><p className="font-medium capitalize text-white">{action.strategy.replaceAll("_", " ")}</p><p className="mt-1 text-sm text-slate-500">{action.details ?? "No additional details"}</p></div><Badge className="shrink-0 border-white/10 bg-white/5 capitalize text-slate-300">{action.status}</Badge></div>)}</div> : <p className="text-sm text-slate-500">No recovery attempts recorded.</p>}</CardContent></Card>
      </div>
      <div className="space-y-6"><AIDecisionCard probability={probability} strategy={strategy} reasoning={reasoning} nextSteps={lastAction?.details ?? "The agent will monitor this transaction and select the next safe action."} /><Card><CardContent><h2 className="text-lg font-semibold">Audit trail</h2><p className="mt-2 text-sm text-slate-500">Every decision is recorded for review.</p><Separator className="my-6" /><AuditTimeline events={auditEvents} /></CardContent></Card></div>
    </div>
  </DetailShell>;
}
